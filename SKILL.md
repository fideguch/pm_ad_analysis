...
Check .claude_ad_memory/ exists?
  |
  |- NO  → Load capability_1 (strategic hearing)
  |- YES → Route by intent:
...
2. No channel context → run Capability 1
3. CSV attached → delegate to pm-ad-operations, synthesize at portfolio level
4. Past history exists → "Last analysis: [X]. Continue or new?"
5. Creative vault >10 items without performance → prompt CSV upload
6. Budget question → Capability 8, delegate within-channel to pm-ad-operations
7. New API docs provided → trigger SOP learning (Layer 1)
8. Amazon data → Load amazon_ads_playbook.md, apply ACOS framework
...
### history_log.jsonl Entry Format

```jsonl
{"ts":"ISO8601","capability":"ngram|creative|fatigue|mmp|asa|tiktok|mmm|incrementality|attribution","channel":"google|meta|asa|tiktok|cross","summary":"1-line finding","confidence":"High|Medium|Low","artifacts":["path"],"delegated_to":"pm-ad-operations|null"}
```

### creative_vault.json Entry Format

```json
{"id":"cr_001","type":"headline","text":"Content","char_count":30,"channel":"google","status":"active","tags":["cta"],"performance":{"impressions":null,"clicks":null,"ctr":null,"conversions":null}}
```
...
- [ ] **Learning Phase**: Campaigns with <50 conversions (Google) or "Learning" (Meta) → EXCLUDE, penalty -2pt
- [ ] **Attribution Windows**: Different windows across channels? → NORMALIZE or FLAG, penalty -1pt
- [ ] **Currency**: All spend in same currency? → If mixed: **BLOCK** — ask user
- [ ] **Seasonality**: Comparable periods? → FLAG if non-comparable, penalty -1pt
- [ ] **LAT/SKAN** (if App): Apply LAT adjustment (35% US, 25% JP default), penalty -1pt if not shown
- [ ] **Budget Shift** (if reallocation): Any channel delta >30%? → **BLOCK** — require PM approval
- [ ] **Creative Sample**: Min 1K impressions, 30 conversions per variant → FLAG insufficient, penalty -1pt
- [ ] **Platform Capability**: P-MAX → skip N-gram; Advantage+ → no audience disaggregation; LAT >50% → downgrade ASA
...
- Score each finding independently (12pt base + penalties)
